...
from Xu_ly.Quan_ly_Ban_hang.Xu_ly_3L import *
...
def Dang_nhap():
    ...
    if request.method == 'POST':
        ...
        Nhan_vien = Dang_nhap_Nhan_vien(
            Cong_ty["Danh_sach_Quan_ly_Ban_hang"], Ten_dang_nhap, Mat_khau)
    ...
    Khung = render_template(
        'Quan_ly_Ban_hang/MH_Dang_nhap.html',
...
def index():
    ...
    return render_template(
        'Quan_ly_Ban_hang/MH_Chinh.html', Dia_chi_MH=Dia_chi_MH, Chuoi_HTML_Nhan_vien=Chuoi_HTML_Nhan_vien, )
...
def Quan_ly_Nhap_Tivi(Ma_so):
    ...
    if Tivi_Chon != None:
        ...
        if request.method == 'POST':
            ...
            print("Đơn giá bán: ", Don_gia_Ban)            
            Tivi_Chon["Don_gia_Ban"] = Don_gia_Ban
            Ghi_Tivi(Tivi_Chon)
            Thong_bao = "Cập nhật thành công đơn giá mới là " + \
                " {:,}".format(Don_gia_Ban).replace(",", ".") 

        Chuoi_HTML_Tivi = Tao_Chuoi_HTML_Tivi(Tivi_Chon, Thong_bao, Don_gia_Ban)
        return render_template(
            'Quan_ly_Ban_hang/MH_Cap_nhap_Tivi.html', Chuoi_HTML_Tivi=Chuoi_HTML_Tivi)
